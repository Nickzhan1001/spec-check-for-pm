...
- 即時同步：WebSocket 維持長連線
- 離線功能：網路中斷時可暫存操作，恢復後自動同步
- 斷線處理：網路中斷時明確提示，恢復後自動重連

**平台建議：**
- 使用 SPA（Single Page Application）架構，適合維持 WebSocket 長連線
...
- 不依賴顏色傳達資訊：同時使用文字或圖標
- 顏色編碼狀態：綠色（#8eba34 - 正常）、藍色（#3a9cd8 - 缺貨待料）、紅色（#f68480 - 通知未到）
...
1. **色彩方案定制**
   - 使用 Element Plus 的 CSS 變數系統定制色彩
   - 主色：`--el-color-primary: #4A90E2`
   - 成功色：`--el-color-success: #8eba34`
   - 警告色：`--el-color-warning: #3a9cd8`（缺貨待料狀態）
   - 錯誤色：`--el-color-danger: #f68480`

2. **狀態顯示組件**
   - 碼頭儀錶板卡片：使用實色背景顯示狀態
     - 正常狀態：`background-color: #8eba34`
     - 缺貨待料：`background-color: #3a9cd8`
     - 通知未到：`background-color: #f68480`
   - 不顯示文字狀態標籤，僅透過顏色識別
   - 使用 ARIA 標籤提供無障礙性支援（`aria-label` 描述狀態）

3. **組件樣式定制**
   - 圓角：調整 Element Plus 組件的 `border-radius` 為 8px
   - 按鈕：使用 Element Plus 的 `el-button` 組件，設定 `type="primary"` 顯示藍色背景
   - 控制欄：使用淺藍色背景（`#E6F7FF` 或 `#4A90E2`）
   - 固定頂部功能列：使用 Element Plus 的 `el-menu` 組件定制
...
5. **無障礙性實作**
   - 狀態卡片使用 `aria-label` 描述狀態（例如：`aria-label="碼頭 1 - 正常狀態"`）
   - 確保顏色對比符合 WCAG AA 標準（4.5:1）
   - 不依賴顏色傳達資訊：雖然視覺上僅使用顏色，但透過 ARIA 標籤確保可訪問性

**開發優先順序：**
1. 基礎色彩方案定制（Element Plus CSS 變數）
2. 碼頭儀錶板狀態顯示組件（實色背景）
3. 固定頂部功能列組件
4. 按鈕與控制欄樣式定制
5. 響應式佈局調整
...
**收貨作業三層架構：**
1. **收貨列表頁：** 顯示所有收貨任務，狀態為「未完成」的車號可點擊
2. **收貨點總表：** 顯示該趟任務的所有配送點，不同狀態以小方框不同顏色顯示
...
- **正常（已選定）：** 綠色實色背景（#8eba34），顯示車牌（車次）與喇叭 icon
...
- **唯碼頭底色隨車輛狀態變化：** 正常（綠色）、缺貨待料（藍色）、通知未到（紅色）
...
- **未排車次：** 點擊下拉選單選擇車次 → 顯示車牌與喇叭 icon，底色變為綠色
- **已選定車次：** 點擊喇叭 icon → 發送通知，顯示提示文字「已通知司機前往碼頭」
- **已選定車次：** 點擊碼頭 → 跳出車次詳細配送單燈箱
- **清空車次：** 在燈箱中選擇「取消」或清空操作 → 碼頭回復未排車次狀態，顯示下拉選單，底色回復預設綠色
...
- **完成出車：** 點擊燈箱內最下方的「完成出車」按鈕後顯示提示文字「已完成出車」，關閉燈箱，車輛從儀錶板移除
- **缺貨待料：** 點擊燈箱內最下方的「缺貨待料」按鈕後更新狀態為缺貨待料，底色改為藍色（#3a9cd8），從儀錶板移除，顯示於未排車次列表，關閉燈箱
- **通知未到：** 點擊燈箱內最下方的「通知未到」按鈕後更新狀態為通知未到，底色改為紅色（#f68480），從儀錶板移除，顯示於未排車次列表，發送通知給運務和運務部後台主管，關閉燈箱
- **取消：** 點擊燈箱內最下方的「取消」按鈕後關閉燈箱，不修改狀態
